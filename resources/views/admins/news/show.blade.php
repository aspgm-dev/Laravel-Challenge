        ...
        <div class="col">
            <div class="card">
                <div class="card-body">
                    ...
                    <img src="{{ asset('admin-lte/dist/img/adminLTELogo.png') }}" class="d-block w-100" height="450px">
                    @else
                    <div id="carouselExampleInterval" class="carousel slide" data-ride="carousel">
                        <div class="carousel-inner">
                            @foreach($news->images()->get() as $item)
                            <div class="carousel-item {{ $loop->first ? ' active' : '' }}" data-interval="10000">
                                <img src="{{ asset('storage/'. $item->image_name) }}" class="d-block w-100"
                                    height="450px">
                            </div>
                            @endforeach
                        </div>
                        <ol class="carousel-indicators mt-5 shadow">
                            @foreach($news->images()->get() as $item)
                            <li data-target="#carouselExampleInterval" data-slide-to="{{ $loop->index }}"
                                class="{{ $loop->first ? ' active' : '' }} ">
                                <img class="d-block w-100 img-fluid shadow-sm"
                                    src="{{ asset('storage/'. $item->image_name) }}" style="height:40px;">
                            </li>
                            @endforeach
                        </ol>
                    </div>
